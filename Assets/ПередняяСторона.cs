...
public class ПередняяСторона : присадкаСтороны {
	...
	public override void GetPrisadka (ref string value)
	{
		for(int i=0; i < stroka.Length;i++)
		{
			...
			{
				value += "XBO X="+ temp[0] +" Y="+ getStrOtcraya().ToString() +" Z=26 D=8 R="+ temp[2] +" x="+ temp[1] +" y=0 N=\"P\" F=5 K=0 P=0\n";
				value += "XBO X="+ temp[0] +" Y=34 Z=13 D=15 R="+ temp[2] +" x="+ temp[1] +" y=0 N=\"P\" F=1 K=0 P=0\n";
			}
			else
				if(stroka[i].Contains("Шкант"))
					...
					if(stroka[i].Contains("Конфирмат"))
						value += "XBO X="+ temp[0] +" Y="+ getStrOtcraya().ToString()+" Z=34 D=5 R="+ temp[2] +" x="+ temp[1] +" y=0 N=\"P\" F=5 K=0 P=0\n";
		}
	}
